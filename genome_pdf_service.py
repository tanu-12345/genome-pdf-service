...
def build_pdf(d):
    ...
    # ── Work Done ─────────────────────────────────────────────
    work_text = str(d.get('polished_work_summary') or d.get('workdone') or d.get('raw_text') or 'No details provided')
    story += [SectionBar('Work Completed Today'), Spacer(1, 5*mm),
              Paragraph(work_text, sbd),
              Spacer(1, 7*mm)]

    # ── Blockers ──────────────────────────────────────────────
    story.append(SectionBar('Blockers & Issues'))
    story.append(Spacer(1, 5*mm))
    bl = str(d.get('polished_blockers') or d.get('blockers') or '').strip()
    ...
    story.append(Spacer(1, 5*mm))
    next_text = str(d.get('polished_next_steps') or d.get('next_steps') or 'N/A')
    story.append(Table([[Paragraph(next_text, sbd)]],
        colWidths=[CWIDTH - 6*mm],
        style=TableStyle([
