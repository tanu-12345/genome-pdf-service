...
def build_pdf(d):
    ...
    lc = Table([
        ...
        [Spacer(1, 2 * mm)],
        [Paragraph(str(pct) + '%', S('PC', fontName='Helvetica-Bold', fontSize=28, textColor=sc, leading=32))],
    ], colWidths=[108 * mm])
    ...
    rc = Table([
        [Paragraph('STATUS', slb)],
        [pill],
        [Spacer(1, 4 * mm)],
        [Paragraph('DATE', slb)],
        [Paragraph(str(d.get('report_date', '')), svl)],
        [Spacer(1, 3 * mm)],
        [Paragraph('TYPE', slb)],
        [Paragraph('Daily EOD Update', svl)],
    ], colWidths=[69 * mm])
    ...
    story += [
        SectionBar('Work Completed Today'),
        Spacer(1, 5 * mm),
        Paragraph(escape(str(work_done_text)), sbd),
        Spacer(1, 7 * mm)
    ]
    ...
    if bl and bl.lower() not in ('none', 'none reported', '', 'n/a'):
        story.append(Paragraph(
            ...
            S('BH', fontName='Helvetica-Bold', fontSize=7.5, textColor=AMBER, leading=10, spaceAfter=4)
        ))
        story.append(Table(
            [[Paragraph(escape(bl), sbd)]],
        ...
        story.append(Table(
            [[Paragraph('No blockers reported for this update period.', sbd)]],
            colWidths=[CWIDTH - 6 * mm],
            style=TableStyle([
    ...
    # ── Next Actions ──────────────────────────────────────────
    next_steps_text = str(d.get('next_steps', 'N/A'))

    story.append(SectionBar('Next Actions Required'))
    story.append(Spacer(1, 5 * mm))
    story.append(Table(
        [[Paragraph(escape(next_steps_text), sbd)]],
        colWidths=[CWIDTH - 6 * mm],
        style=TableStyle([
    ...
    bm = Table([[
        Paragraph('<b>Report No.</b>  ' + escape(report_no), ssm),
        Paragraph('<b>Generated by</b>  Genome AI Operations System', ssm),
        Paragraph(
            '<b>Classification</b>  Confidential',
            S('CR', fontName='Helvetica', fontSize=7.5, textColor=MID_GREY, leading=11, alignment=2)
        ),
